...
class MtkExtractor(BaseExtractor):
    ...
    def _get_entries(fd: BinaryIO, header: _Header, file_size: int):
        result = []
        fd.seek(
            file_size - HEADER_SIZE - header.entries_count * ENTRY_SIZE, io.SEEK_SET
        )

        data = Utils.mtk_header_shuffle(
            fd.read(header.entries_count * ENTRY_SIZE),
            header_size=header.entries_count * ENTRY_SIZE,
        )

        for entries in range(0, header.entries_count):
            entry = _Entry(
                data[entries * ENTRY_SIZE : (entries * ENTRY_SIZE) + ENTRY_SIZE]
            )
            result.append(entry)
