...
from exceptions import (
    OpsExtractorUnsupportedCryptoSettingsError,
    UtilsNoSupportedHashAlgorithmError,
    UtilsFileNotFoundError,
)
...
class OpsExtractor(BaseExtractor):
    ...
    def _find_xml_crypto_data(fd: BinaryIO, file_size: int) -> Tuple[bytes, int]:
        ...
        fd.seek(
            file_size
            - HEADER_SIZE
            - (xml_length + (HEADER_SIZE - (xml_length % HEADER_SIZE)))
        )
        return fd.read(
            xml_length + (HEADER_SIZE - (xml_length % HEADER_SIZE))
        ), xml_length
    ...
    def _parse_xml_item(element: ET.Element) -> _Payload:
        ...
        payload.length = (
            int(value) * HEADER_OFFSET
            if (value := element.attrib.get("SizeInSectorInSrc", None))
            else payload.rlength
        )
        ...
        if length > 0xF:
            for ptr in range(0, length, 0x10):
                ...
                out.extend(
                    struct.pack(
                        "<4I",
                        *[custom_key[index] ^ data[index] for index in range(0, 4)],
                    )
                )
                custom_key = data
                length = length - 0x10
    ...
    def extract(self, fd: BinaryIO, output_dir: Path, file_size):
        ...
        for index, job in enumerate(jobs):
            ...
            if (
                job.payload.sha256
                and job.payload.sha256 != ""
                and not job.payload.sparse
            ):
            ...
            try:
                if Utils.validate_checksum(
                    job.payload.__getattribute__(algorithm),
                    dst_path,
                    algorithm,
                    fill_func=self._digest_fill,
                ):
                    self.logger.debug(
                        f"Check {job.payload.filename} success! Algorithm {algorithm}: verified"
                    )
                else:
