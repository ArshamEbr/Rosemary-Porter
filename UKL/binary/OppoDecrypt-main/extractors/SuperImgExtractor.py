...
from core.models.super import (
    MetadataHeaderModel,
    MetadataGeometryModel,
    MetadataPartitionModel,
    MetadataExtentModel,
    MetadataPartitionGroupModel,
    MetadataBlockDeviceModel,
    MetadataTableDescriptorModel,
)
...
class _Metadata:
    ...
    block_devices: list[MetadataBlockDeviceModel] = dataclasses.field(
...
class SuperImgExtractor(BaseExtractor):
    ...
    def _read_primary_geometry(fd: BinaryIO) -> MetadataGeometryModel:
        ...
        return (
            ...
            else MetadataGeometryModel(fd.read(METADATA_GEOMETRY_SIZE))
        )
    ...
    def _read_metadata_header(self, fd: BinaryIO, metadata: _Metadata):
        for index, offset in enumerate(offsets := metadata.get_offsets()):
            ...
            header = MetadataHeaderModel(
                fd.read(typing.cast(int, MetadataHeaderModel.size))
            )
            for field in ["partitions", "extents", "groups", "block_devices"]:
                header.__setattr__(
                    field,
                    MetadataTableDescriptorModel(
                        fd.read(typing.cast(int, MetadataTableDescriptorModel.size))
                    ),
                )
            ...
            if header.magic != METADATA_HEADER_MAGIC:
                ...
                if check_index > len(offsets):
                    ...
                    self.logger.information(
                        f"Read Backup header by offset 0x{offsets[check_index]:x}"
                    )
                    continue
    ...
    def _read_metadata(self, fd: BinaryIO) -> None:
        ...
        try:
            super_device: MetadataBlockDeviceModel = typing.cast(
                MetadataBlockDeviceModel, iter(metadata.block_devices).__next__()
            )
            if (
                metadata.metadata_region
                > super_device.first_logical_sector * SECTOR_SIZE
            ):
                raise SuperImgExtractorError(
    ...
    def _write_extent_to_file(
        self, in_fd: BinaryIO, out_fd: BinaryIO, offset: int, size: int
    ):
        in_fd.seek(offset, io.SEEK_SET)

        for block in self._read_chunk(
            in_fd, self._metadata.geometry.logical_block_size
        ):
            if size == 0:
                break
    ...
    def extract(self, fd: BinaryIO, output_dir: Path, file_size) -> PayloadModel:
        ...
        return PayloadModel(
            input_file=extract_files if extract_files else None, output_dir=output_dir
        )

    @ConfirmationExecution(
        "Run extract partitions image from super partition?", forced=True
    )
    def run(self, payload: PayloadModel) -> PayloadModel:
        ...
        try:
            with open(payload.input_file, "rb") as fd:
                ...
                choices = [
                    Choice(value=self._metadata.partitions, name="All", enabled=True)
                ]
                choices.extend(
                    [
                        Choice(
                            value=item,
                            name=item.name[:-2]
                            if item.name.endswith(("_a", "_b"))
                            else item.name,
                            enabled=False,
                        )
                        for item in self._metadata.partitions
                        if item.num_extents != 0
                    ]
                )
                self._metadata.partitions = Cli.get_choice_extraction_partitions(
                    choices
                )

                payload = self.extract(
                    fd, payload.output_dir, payload.input_file.stat().st_size
                )
