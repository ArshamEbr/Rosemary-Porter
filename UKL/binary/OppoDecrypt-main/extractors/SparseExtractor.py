...
class SparseExtractor(BaseExtractor):
    ...
    def _unsparse(self, in_fd: BinaryIO, out_fd: BinaryIO, header: _SparseHeader):
        ...
        while chunks > 0:
            chunk_header = _SparseChunkHeader(
                in_fd.read(self._SPARSE_CHUNK_HEADER_SIZE), header
            )
            ...
            match chunk_header.chunk_type:
                case SparseChunkTypeEnum.CHUNK_TYPE_RAW:
                    if header.chunk_header_size > self._SPARSE_CHUNK_HEADER_SIZE:
                        in_fd.seek(
                            header.chunk_header_size - self._SPARSE_CHUNK_HEADER_SIZE, 1
                        )
                ...
                case SparseChunkTypeEnum.CHUNK_TYPE_FILL:
                    ...
                    out_fd.write(
                        struct.pack("B", 0) * (chunk_header.sector_size << 0x9)
                    )
    ...
    def is_sparse(self, fd: BinaryIO) -> bool:
        ...
        _fmt = "<I"
        (magic_number,) = struct.unpack(_fmt, fd.read(struct.calcsize(_fmt)))
    ...
    def extract(self, fd: BinaryIO, output_dir: Path, file_size) -> PayloadModel:
        ...
        with open(
            (dst_path := output_dir / self._out_filename),
            "rb+" if dst_path.exists() else "wb",
            buffering=self._BUFFER_PAGE_SIZE,
        ) as out:
    ...
    def run(self, payload: PayloadModel) -> PayloadModel:
        if isinstance(payload.input_file, Path) and not payload.input_file.exists():
            ...
            find_images = list(
                filter(
                    lambda x: x.name != self._out_filename,
                    list(payload.output_dir.glob(payload.search_pattern)),
                )
            )
        ...
        try:
            for input_file in payload.input_file:
                ...
                with open(input_file, "rb") as fd:
                    result = self.extract(
                        fd, payload.output_dir, input_file.stat().st_size
                    )

            out_file = (
                result.input_file.parent
                / f"{result.input_file.name.replace('.unsparse', '')}"
            )
            if out_file.exists():
