...
class OfpMtkConfiguration:
    ...
    def __post_init__(self):
        ...
        for item in self.keys:
            ...
            if (
                not obs_key
                and (aes_key and aes_key != "")
                and (aes_iv and aes_iv != "")
            ):
                result.append(
                    CryptoCredential(
                        key=aes_key.encode("utf-8"), iv=aes_iv.encode("utf-8")
                    )
                )
                continue
            ...
            if (
                ...
                key = (
                    hashlib.md5(
                        self._shuffle(
                            bytearray.fromhex(obs_key), bytearray.fromhex(aes_key)
                        )
                    ).hexdigest()[:KEY_SIZE]
                ).encode()
                iv = (
                    hashlib.md5(
                        self._shuffle(
                            bytearray.fromhex(obs_key), bytearray.fromhex(aes_iv)
                        )
                    ).hexdigest()[:KEY_SIZE]
                ).encode()
