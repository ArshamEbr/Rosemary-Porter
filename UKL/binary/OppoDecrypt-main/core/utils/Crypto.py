...
class Crypto:
    ...
    def decrypt_aes_cfb(crypto_credential: CryptoCredential, data: bytes):
        return AES.new(
            crypto_credential.key,
            AES.MODE_CFB,
            iv=crypto_credential.iv,
            segment_size=128,
        ).decrypt(data)
    ...
    def ops_key_update(iv1, asbox, offset: int = 4):
        def get_sbox_value(sbox_offset):
            ...
            return struct.unpack(
                fmt, SBOX_BIN[sbox_offset : sbox_offset + struct.calcsize(fmt)]
            )[0]
        ...
        def calculate(input_list):
            ...
            for index in range(0, len(input_list)):
                if (position := index + 2) < 4:
                    _i1 = get_sbox_value(
                        ((input_list[position] >> 0x10) & 0xFF) * 8 + count
                    )
                else:
                    if count > 2:
                        _i1 = get_sbox_value(
                            ((input_list[0] >> 0x8) & 0xFF) * 8 + count
                        )
                    else:
                        _i1 = get_sbox_value(
                            ((input_list[0] >> 0x10) & 0xFF) * 8 + count
                        )
                        count += 1
                ...
                if (position := (index + 1) % 4) != 0:
                    ...
                    _i2 = get_sbox_value(
                        ((input_list[position + 1] >> 0x10) & 0xFF) * 8 + 2
                    )
        ...
        for index in range(len(tmp)):
            i1 = (
                get_sbox_value(((tmp[(index + 2) % 4] >> 0x10) & 0xFF) * 0x8) & 0xFF0000
            )
            i2 = (
                get_sbox_value(((tmp[(index + 1) % 4] >> 0x8) & 0xFF) * 0x8 + 0x1)
                & 0xFF00
            )
