...
@inject
def main(
    service: IBaseExtractService = Provide[ApplicationContainer.extract_service],
    **kwargs,
) -> None:
...
def create_parser() -> argparse.ArgumentParser:
    ...
    _parser.add_argument(
        "--sparse", type=bool, default=False, action=argparse.BooleanOptionalAction
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        ...
        if debug:
            configuration["log"]["handlers"] = set_debug_mode(
                configuration["log"]["handlers"]
            )
