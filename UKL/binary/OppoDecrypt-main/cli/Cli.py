...
class Cli:
    ...
    def get_super_map_path(input_files: list[Path]) -> Path:
        return inquirer.filepath(
            ...
            validate=SupersImgCompareCsvValidator(
                list_files=input_files, message="Input file is not a valid"
            ),
            only_files=True,
            mandatory=True,
    ...
    @staticmethod
    def get_choice_extraction_partitions(
        choices: list[Choice],
    ) -> list[MetadataPartitionModel]:
    ...
    def get_extract_folder(output_folder: Path):
        return inquirer.filepath(
            ...
            validate=PathValidator(
                is_file=False, is_dir=True, message="Input is not a dir"
            ),
