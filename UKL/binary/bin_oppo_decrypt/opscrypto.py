...
class QCSparse:
    ...
    def readheader(self, offset):
        ...
        if self.file_hdr_sz != 28:
            self.error(
                "The file header size was expected to be 28, but is %u."
                % self.file_hdr_sz
            )
            return False
        if self.chunk_hdr_sz != 12:
            self.error(
                "The chunk header size was expected to be 12, but is %u."
                % self.chunk_hdr_sz
            )
            return False
    ...
    def get_chunk_size(self):
        if self.total_blks < self.offset:
            self.error(
                "The header said we should have %u output blocks, but we saw %u"
