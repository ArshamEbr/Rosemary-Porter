...
class Volume:
    ...
    def __init__(self, stream, offset=0, ignore_flags=False, ignore_magic=False):
        ...
        if not ignore_magic and self.superblock.s_magic != 0xEF53:
            raise MagicError(
                "Invalid magic value in superblock: 0x{magic:04X} (expected 0xEF53)".format(
    ...
    # 06.10.2023 -->
    @property
    def mkfs_time(self):  # When the filesystem was created, in seconds since the epoch
        return self.superblock.s_mkfs_time  # 1362301382
        # import datetime
        # ts_epoch = self.superblock.s_mkfs_time  # 1362301382
        # ts = datetime.datetime.fromtimestamp(ts_epoch).strftime('%Y-%m-%d %H:%M:%S')
        # return ts

    # 06.10.2023 <--
    # 12.10.2023 -->
    def lastcheck(self):  # Time of last check, in seconds since the epoch.
        return self.superblock.s_lastcheck

    def checkinterval(self):  # Maximum time between checks, in seconds.
        return self.superblock.s_checkinterval

    # 12.10.2023 <--
    # 20.10.2023 -->
    def mtime(self):  # Mount time, in seconds since the epoch.
        return self.superblock.s_mtime

    def wtime(self):  # Write time, in seconds since the epoch.
        return self.superblock.s_wtime

    def volume_name(self):
        return self.superblock.s_volume_name.decode("utf-8")

    def last_mounted(self):
        return self.superblock.s_last_mounted.decode("utf-8")
...
class Inode:
    ...
    def __repr__(self):
        if self.inode_idx != None:
            ...
            return (
                "{type_name:s}(offset = 0x{offset:X}, volume_uuid = {uuid!r:s})".format(
                    offset=self.offset,
                    type_name=type(self).__name__,
                    uuid=self.volume.uuid,
                )
    ...
    def _parse_xattrs(self, raw_data, offset, prefix_override={}):
        ...
        while i < len(raw_data):
            ...
            if not xattr_entry.e_name_index in prefixes:
                raise Ext4Error(
                    "Unknown attribute prefix {prefix:d} in inode {inode:d}".format(
                        inode=self.inode_idx, prefix=xattr_entry.e_name_index
                    )
                )
    ...
    def get_inode(self, *relative_path, decode_name=None):
        ...
        for i, part in enumerate(relative_path):
            if not self.volume.ignore_flags and not current_inode.is_dir:
                ...
                raise Ext4Error(
                    "{current_path!r:s} (Inode {inode:d}) is not a directory.".format(
                        current_path=current_path, inode=inode_idx
                    )
                )
            ...
            if inode_idx == None:
                ...
                raise FileNotFoundError(
                    "{part!r:s} not found in {current_path!r:s} (Inode {inode:d}).".format(
    ...
    def size_readable(self):
        if self.inode.i_size < 1024:
            ...
            return "{size:.2f} {unit:s}".format(
                size=self.inode.i_size / (1024**unit_idx), unit=units[unit_idx - 1]
            )
    ...
    def xattrs(
        ...
        if check_inline and inline_data_length > ctypes.sizeof(ext4_xattr_ibody_header):
            ...
            if force_inline or xattrs_header.h_magic == 0xEA020000:
                offset = (
                    4 * ((ctypes.sizeof(ext4_xattr_ibody_header) + 3) // 4)
        ...
        if check_block and self.inode.i_file_acl != 0:
            ...
            offset = (
                4 * ((ctypes.sizeof(ext4_xattr_header) + 3) // 4)
            )  # The ext4_xattr_entry following the header is aligned on a 4-byte boundary
            for xattr_name, xattr_value in self._parse_xattrs(
