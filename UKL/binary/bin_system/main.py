...
class Patch:
    ...
    def split_zimg(self, zimg_file):
        ...
        if data[0] != 0x016F2818:
            raise Exception("ERROR: Didn't find IMG magic number")
        zimg_size = data[2]
        zimg_file.seek(0)
        d = zimg_file.read()
    ...
    def kernel_work(self, gz_data):
        p7z_pack = [sz, "a", "dummy", "-tgzip", "-si", "-so", "-mx5", "-mmt4"]
        ...
        if b"skip_initramfs" not in kernel_data:
            raise Exception("ERROR: Didn't find skip_initramfs, no need to patch.")
        printi("Patching kernel data...")
        kernel_data = kernel_data.replace(
            b"skip_initramfs",  # basically everything
            b"want_initramfs",
        )  # we were doing is for this
    ...
    def join_zimg(self):
        ...
        with io.BytesIO() as new_zimg_file:
            ...
            # Write new gz size
            new_zimg_file.write(struct.pack("I", self.gz_begin + self.new_gz_size - 4))
