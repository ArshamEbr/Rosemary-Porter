...
class ext4_file_header(object):
    def __init__(self, buf):
        (
            ...
            self.total_chunks,
            self.crc32,
        ) = struct.unpack("<I4H4I", buf)


class ext4_chunk_header(object):
    def __init__(self, buf):
        (self.type, self.reserved, self.chunk_size, self.total_size) = struct.unpack(
            "<2H2I", buf
        )
...
class Extractor(object):
    ...
    def __ext4extractor(self):
        # 25.04.2021 -->
        config_dir = (
            os.path.dirname(self.EXTRACT_DIR)
            + os.sep
            + "config"
            + os.sep
            + self.FileName
        ...
        self.__appendf(
            "make_ext4fs -J -T -1 -S ./file_contexts -C ./fs_config -l "
            + str(os.path.getsize(self.OUTPUT_IMAGE_FILE))
            + " -a /"
            + self.FileName
            + ' "$outdir"/'
            + self.FileName
        ...
        self.__appendf(
            "make_ext4fs -s -J -T -1 -S ./file_contexts -C ./fs_config -l "
            + str(os.path.getsize(self.OUTPUT_IMAGE_FILE))
            + " -a /"
            + self.FileName
            + ' "$outdir"/'
            ...
            for entry_name, entry_inode_idx, entry_type in root_inode.open_dir():
                ...
                for i in list(entry_inode.xattrs()):
                    if i[0] == "security.selinux":
                        ...
                        if raw_cap[1] > 65535:
                            cap = "" + str(
                                hex(int("%04x%04x" % (raw_cap[3], raw_cap[1]), 16))
                            )
                        else:
                            cap = "" + str(
                                hex(
                ...
                if entry_inode.is_dir:
                    ...
                    if cap == "" and con == "":
                        ...
                        if cap == "":
                            ...
                            if con == "":
                                self.fsconfig.append(
                                    "%s %s %s %s" % (tmppath, uid, gid, mode + cap)
                                )
                            else:
                                self.fsconfig.append(
                                    "%s %s %s %s" % (tmppath, uid, gid, mode + cap)
                    ...
                    if tmppath.find(" ", 1, len(tmppath)) > 0:
                        ...
                        tmppath = tmppath.replace(" ", "_")
                    # 08.05.2023 -->
                    self.__createtar_symlink(
                        tar_file_symlink, tmppath, uid, gid, int(mode), link_target
                    )
                    # <-- 08.05.2023
                    # 20.10.2023 -->
                    self.linkmass.append(
                        "%s -> %s %s %s %s" % (tmppath, link_target, uid, gid, mode)
                    )
                    self.fsconfig_e2fsdroid.append(
                        "%s %s %s %s" % (tmppath, uid, gid, mode)
                    )
                    # <-- 20.10.2023
                    try:
                        ...
                        if not self.Listing:
                            ...
                            if os.name == "nt":
                                with open(target.replace("/", os.sep), "wb") as out:
                                    ...
                                    os.system(
                                        'attrib +s "%s"' % target.replace("/", os.sep)
                                    )
                        if not all(c in string.printable for c in link_target):
                            pass
                        if (
                            entry_inode_path[1:] == entry_name
                            or link_target[1:] == entry_name
                        ):
                            self.symlinks.append(
                                "%s %s" % (link_target, entry_inode_path[1:])
                            )
                        else:
                            self.symlinks.append(
                                "%s %s"
                                % (link_target, self.FileName + entry_inode_path)
                            )
                    except:
                        try:
                            ...
                            target = self.EXTRACT_DIR + entry_inode_path.replace(
                                " ", "_"
                            )
                            tmppath = self.FileName + entry_inode_path
                            if tmppath.find(" ", 1, len(tmppath)) > 0:
                                self.__appendf(tmppath, spaces_file)
                                tmppath = tmppath.replace(" ", "_")
                            if link_target and all(
                                c in string.printable for c in link_target
                            ):
                                if cap == "" and con == "":
                                    self.fsconfig.append(
                                        "%s %s %s %s %s"
        ...
        with open(self.OUTPUT_IMAGE_FILE, "rb") as file:
            ...
            self.fsconfig_e2fsdroid.append(
                "%s %s %s %s" % ("/", rootdiruid, rootdirgid, rootdirperm)
            )
            self.fsconfig.append(
                "%s %s %s %s" % ("/", rootdiruid, rootdirgid, rootdirperm)
            )
            ...
            if self.context:  # 11.05.18
                ...
                self.ggftxt.sort()
                self.__appendf(
                    "\n".join(self.ggftxt), gg_f_txt
                )  # закоментируй если считаешь, что работает не так, либо давай обсудим и допилим
                self.ggdtxt.sort()
                self.__appendf(
                    "\n".join(self.ggdtxt), gg_d_txt
                )  # закоментируй если считаешь, что работает не так, либо давай обсудим и допилим
                ...
                self.__appendf(
                    "\n".join(self.avtoftxt), avto_f_txt
                )  # закоментируй если считаешь, что работает не так, либо давай обсудим и допилим
                self.fsconfig_e2fsdroid.sort()
                self.__appendf(
                    "\n".join(self.fsconfig_e2fsdroid), fs_config_file_e2fsdroid
                )  # 20.10.2023
    ...
    def checkSignOffset(self, file):
        ...
        if size <= 1048576:
            ...
            mm = mmap.mmap(
                file.fileno(), 1048576, access=mmap.ACCESS_READ
            )  # 1048576=1Mb
    ...
    def ___sar_detect(self, img_file):
        ...
        with open(img_file, "rb") as file:
            ...
            for entry_name, entry_inode_idx, entry_type in root.open_dir():
                ...
                if entry_inode.is_symlink and (
                    entry_name == "init" or entry_name == "d"
                ):
                    self.isSAR = True
        if self.isSAR == True:
        ...
        if target_type == "simg":
            print(
                ...
                % (
                    ...
                    os.path.basename(target).rsplit(".", 1)[0] + ".raw.img",
                )
            )
...
if __name__ == "__main__":
    ...
    if args.outdir:
        ...
        OUTDIR = (
            os.path.realpath(os.path.dirname(INPUT_IMAGE))
            + os.sep
