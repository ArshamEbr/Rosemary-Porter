...
class Qfil(object):
    ...
    def __parseXML(self, file_path):
        ...
        for item in e.findall("program"):
            if item.get("label") == "system" or item.get("label") == "vendor":
    ...
    def __copy_img(self, filename, seek):
        ...
        try:
            ...
            print(
                'File "%s" not found in directory "%s". Unpacking is\'t possible.'
                % (os.path.basename(filename), self.target_dir)
            )
            sys.exit(0)
    ...
    def convert(self, file_path):
        ...
        self.out_file = open(
            self.target_dir + os.sep + self.name, "wb+", buffering=self.buffer
        )
...
if __name__ == "__main__":
    Qfil().convert("/data/local/UnpackerQfil/rawprogram0.xml")
