...
def LoadInfoDict(input_file, input_dir=None):
    ...
    if input_dir is not None:
        ...
        if "system_base_fs_file" in d:
            ...
            if os.path.exists(system_base_fs_file):
                ...
                print(
                    "Warning: failed to find system base fs file: %s"
                    % (system_base_fs_file,)
                )
        ...
        if "vendor_base_fs_file" in d:
            ...
            if os.path.exists(vendor_base_fs_file):
                ...
                print(
                    "Warning: failed to find vendor base fs file: %s"
                    % (vendor_base_fs_file,)
                )
    ...
    if d.get("no_recovery", False) == "true":
        ...
        d["fstab"] = LoadRecoveryFSTab(
            read_helper, d["fstab_version"], d.get("system_root_image", False)
        )
...
def _BuildBootableImage(sourcedir, fs_config_file, info_dict=None, has_ramdisk=False):
    ...
    if info_dict.get("boot_signer", None) == "true" and info_dict.get(
        "verity_key", None
    ):
...
def GetBootableImage(name, prebuilt_name, unpack_dir, tree_subdir, info_dict=None):
    ...
    # for recovery.
    has_ramdisk = (
        info_dict.get("system_root_image") != "true"
        or prebuilt_name != "boot.img"
        or info_dict.get("recovery_as_boot") == "true"
    )
...
def GetMinSdkVersionInt(apk_name, codename_to_api_level_map):
    ...
    try:
        ...
        if version in codename_to_api_level_map:
            ...
            raise ExternalError(
                "Unknown minSdkVersion: '%s'. Known codenames: %s"
...
def ReadApkCerts(tf_zip):
    ...
    for line in tf_zip.read("META/apkcerts.txt").split("\n"):
        ...
        if not line:
            continue
        m = re.match(
            r'^name="(.*)"\s+certificate="(.*)"\s+' r'private_key="(.*)"$', line
        )
        if m:
...
def ParseOptions(
    argv, docstring, extra_opts="", extra_long_opts=(), extra_option_handler=None
):
    ...
    def __init__(
        ...
        if version is None:
            ...
            if OPTIONS.info_dict:
                version = max(
                    int(i)
                    for i in OPTIONS.info_dict.get("blockimgdiff_versions", "1").split(
                        ","
                    )
                )
        self.version = version

        b = blockimgdiff.BlockImageDiff(
            tgt,
            src,
