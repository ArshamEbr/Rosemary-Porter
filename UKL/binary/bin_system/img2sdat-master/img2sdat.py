...
try:
    ...
    print(
        "    [version]: transfer list version number, will be asked by default - more info on xda thread)\n"
    )
...
def __FindStringInByteFile(word, file):
    ...
    with open(file, "r+b") as f:
        for line in f:
            if findword in line:
                rez = line.__str__()[
                    2 : len(line.__str__()) - 3
                ]  # получаем строку без b' вначале и \n' в конце строки
    ...
    if len(sys.argv) > 2 and len(sys.argv) < 4:
        if sys.argv[len(sys.argv) - 1].isdigit():
            if int(sys.argv[len(sys.argv) - 1]) < 5:
                ...
                outdir = (
                    os.path.realpath(os.path.dirname(sys.argv[1]))
                    + os.sep
                ...
                versionsdk = __FindStringInByteFile(
                    "ro.build.version.sdk=", sys.argv[1]
                ).__str__()
