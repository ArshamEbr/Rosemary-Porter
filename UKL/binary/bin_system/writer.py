...
class SimgWriter(object):
    ...
    def _print_state(self, pfx, debug):
        if self.debug >= debug:
            print(
                "%snchunks=%d, nblocks=%d, npadblocks=%d, ctype=%s, cval=%r, csize=%d, len(buf)=%d, tell()=%d"
                % (
                    pfx,
                    self.nchunks,
                    self.nblocks,
                    self.npadblocks,
                    self.ctype and self.ctype.name,
                    self.cval,
                    self.csize,
                    len(self.buf),
                    self.tell(),
    ...
    def close(self):
        if self.buf:
            raise RuntimeError(
                "%d leftover bytes (data written must be a multiple of blocksize %d)"
                % (len(self.buf), self.blocksize)
            )
    ...
    def _close_chunk(self):
        ...
        if self.ctype == SparseChunkType.RAW:
            ...
            self.outf.seek(
                -databytes - self.chunk_header_size, SEEK_CUR
            )  # return to where chunk header belongs
        ...
        self.outf.write(
            pack(
                ...
                0,
                self.csize,
                self.chunk_header_size + databytes,
            )
        )
