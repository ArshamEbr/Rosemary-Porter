...
class SplitAPP:
    ...
    def _write_to_file(
        self, out_file_path: Path, *, start: int, size: int, buffer_size: int
    ):
        ...
        with open(out_file_path, "wb") as out:
            ...
            while write_byte != size:
                ...
                buffer_size = (
                    buffer_size
                    if size - write_byte > buffer_size
                    else size - write_byte
                )
    ...
    def generate_new_path(path: Path, uniq_new_name: bool = True) -> Generator:
        new_name_gen = (
            lambda p, c: (
                Path(p.parent / f"{p.stem}_{index}{p.suffix}") for index in range(c)
            )
        )(path, 1000)
        ...
        try:
            while True:
                yield new_name_func(
                    Path(path.parent / new_name_gen.__next__()), new_name_gen
                )
    ...
    def rebuild_duplicate_path(partitions: List[ImageMetada]) -> List[ImageMetada]:
        ...
        for index in duplicates_index:
            generate_new_path_gen = SplitAPP.generate_new_path(
                partitions[index].path, uniq_new_name=False
            )
    ...
    def info(self):
        for partition in self._read_partitions():
            print(
                f"Partition: {partition.name} Start: {hex(partition.start)} Size: {hex(partition.size)}"
            )
...
def create_parser():
    ...
    required.add_argument(
        "-f", "--filename", required=True, help="Path to update.app file"
    )
    ...
    optional.add_argument(
        ...
        nargs="*",
        metavar=("img1", "img2"),
        help="List of img files to extract",
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 5:
        ...
        SplitAPP(input_file=namespace.filename, out_dir=namespace.out_dir).extract(
            buffer_size=buffer_size, list_partition=partition_list
        )
